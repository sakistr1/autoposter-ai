...
# Προαιρετικά: register templates (idempotent)
...
step "previews/render (raw)" 'curl -s -w "\n%{http_code}\n" -X POST "$BASE/previews/render" "${AHDR[@]}" -H "Content-Type: application/json" -d @/tmp/render.json | tee /tmp/render.out'

echo "---- parsed render ----"
REND_CODE=$(tail -n1 /tmp/render.out | tr -d '\r')
REND_JSON=$(sed '$d' /tmp/render.out)
echo "$REND_JSON" | jq . 2>/dev/null || echo "$REND_JSON"

if [ "$REND_CODE" != "200" ]; then
  echo "Render FAILED (HTTP $REND_CODE) – δείξε το παραπάνω JSON (.detail) για το γιατί."
  echo "STOP."
  exit 0
fi

PID=$(echo "$REND_JSON" | jq -r '.preview_id // empty')
if [ -z "$PID" ]; then
  PURL=$(echo "$REND_JSON" | jq -r '.preview_url // .url // empty')
  [ -n "$PURL" ] && PID=$(basename "$PURL" .png)
fi
echo "PID=$PID"
if [ -z "$PID" ]; then
  echo "Δεν βρέθηκε preview_id – STOP εδώ."
  exit 0
fi

# ---------- COMMIT ----------
jq -n --arg pid "$PID" --arg c "ok!" '{preview_id:$pid, caption:$c}' >/tmp/commit.json
step "previews/commit (raw)" 'curl -s -w "\n%{http_code}\n" -X POST "$BASE/previews/commit" "${AHDR[@]}" -H "Content-Type: application/json" -d @/tmp/commit.json | tee /tmp/commit.out'

echo "---- parsed commit ----"
COMM_CODE=$(tail -n1 /tmp/commit.out | tr -d '\r')
COMM_JSON=$(sed '$d' /tmp/commit.out)
echo "$COMM_JSON" | jq . 2>/dev/null || echo "$COMM_JSON"

FINAL=$(echo "$COMM_JSON" | jq -r '(.final_url // .urls[0] // .image_url // .media_urls[0] // empty)')
echo "FINAL=$FINAL"
...
echo "DB_UID=$DB_UID"
if [ -z "$DB_UID" ]; then
  echo "!!! ΔΕΝ βρέθηκε user στη $DB για $EMAIL — έλεγξε ότι δείχνεις στη σωστή βάση (pwd; ls -l autoposter.db; sqlite3 autoposter.db \"SELECT id,email FROM users;\")"
  echo "STOP uploads."
  exit 0
fi
